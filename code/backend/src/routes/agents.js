...
router.get('/', paginationValidators.list, validationHandler, async (req, res) => {
  try {
    ...
    const options = { page, limit, status, category, keyword };
...
router.post('/batch-import', async (req, res) => {
  try {
    ...
    // 从文件导入
    const result = await agentService.batchImport([]);
...
router.patch('/:id/status', agentValidators.statusUpdate, validationHandler, async (req, res) => {
  try {
    ...
    const agent = await agentService.updateAgentStatus(id, {
      status,
      task_id,
      message,
      metadata
...
router.post('/:id/status-report', agentValidators.statusReport, validationHandler, async (req, res) => {
  try {
    ...
    // 更新智能体状态
    await agentService.updateAgentStatus(id, { status, task_id: current_task_id, message });
    
    // 更新当前任务关联
    if (current_task_id) {
      await agentService.updateAgentCurrentTask(id, current_task_id);
    }
